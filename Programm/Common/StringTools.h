...
namespace IBDS
{
	...
	// String + Vector3D
	std::string operator + (const std::string str, const Eigen::Vector3d &v);
	// Vector3D + String
	std::string operator + (const Eigen::Vector3d &v, const std::string str);
	...
	std::string str(const Eigen::Vector3d &v);
	std::string str(const Eigen::Matrix3d& m);
